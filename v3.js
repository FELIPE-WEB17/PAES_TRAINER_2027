...
function v3Exams(){resetState();navActive('exams');const latest=latestByTest();app.innerHTML=`<div class="v3pageHead"><div><div class="eyebrow">SIMULACIÓN PAES</div><h1>Ensayos</h1><p>Practica con tiempo, estructura completa y puntaje estimado referencial.</p></div><div class="toolbar compact"><button class="secondary" onclick="v3DemreLibrary()">📚 Material DEMRE 2020–2025</button><button class="secondary" onclick="generatorSetup()">✨ Crear ensayo nuevo</button></div></div><section class="v3examGrid">${visibleTests().map(t=>{const l=latest[t.id];return `<article class="card v3examCard"><div><div class="eyebrow">${t.minutes} MIN · ${t.questions.length} PREGUNTAS</div><h2>${esc(t.title)}</h2><p>${l?`Último: <b>${l.score} pts</b> · ${l.correct}/${l.scored}`:'Aún no tienes resultado registrado.'}</p></div><div class="toolbar compact"><button class="btn" onclick="startTest('${t.id}')">Ensayo base</button><button class="secondary" onclick="startGenerated('${t.id}')">Nuevo ensayo</button></div></article>`}).join('')}</section><p class="tiny v3demreNote">Las preguntas de PAES Trainer son material original de práctica. La organización de contenidos y habilidades busca mantenerse alineada con los temarios DEMRE vigentes; los puntajes son estimaciones de estudio.</p>`;}
const SCORE_KEY='paesOfficialScoresV33';
function getOfficialScores(){return safeParse(localStorage.getItem(scopedKey(SCORE_KEY)),{});}
function saveOfficialScore(field,value){const d=getOfficialScores();const n=parseInt(value,10);if(Number.isFinite(n)&&n>=100&&n<=1000)d[field]=n;else delete d[field];localStorage.setItem(scopedKey(SCORE_KEY),JSON.stringify(d));v3Progress();}
function scoreTile(label,key,icon){const d=getOfficialScores(),v=d[key];return `<article class="officialScoreTile"><div class="scoreIcon">${icon}</div><div class="scoreValue">${Number.isFinite(v)?v:'–'}</div><div class="scoreName">${label}</div><input aria-label="${label}" inputmode="numeric" type="number" min="100" max="1000" placeholder="Ingresar" value="${Number.isFinite(v)?v:''}" onchange="saveOfficialScore('${key}',this.value)"></article>`;}
function v3Progress(){navActive('progress');resetState();const p=getActiveProfile();const d=getOfficialScores();app.innerHTML=`<div class="v3pageHead"><div><div class="eyebrow">MI PROGRESO</div><h1>Mis puntajes y diagnóstico</h1><p>Registra tus puntajes oficiales o metas y compáralos con tu evolución dentro de PAES Trainer.</p></div><button class="secondary" onclick="diagnostic()">Ver diagnóstico detallado</button></div>
<section class="officialScorePanel"><div class="scorePanelHead"><div><span>🎯</span><h2>Mis puntajes</h2></div><small>Rango referencial PAES: 100–1000</small></div><div class="officialScoreGrid">${scoreTile('Competencia Lectora','lectora','📖')}${scoreTile('Competencia Matemática 1 (M1)','m1','➗')}${scoreTile('Ciencias','ciencias','🧪')}${scoreTile('Historia y Ciencias Sociales','historia','🌎')}${scoreTile('Competencia Matemática 2 (M2)','m2','📐')}${scoreTile('Puntaje NEM','nem','🎓')}${scoreTile('Puntaje Ranking','ranking','🏅')}</div></section>
<section class="grid three"><article class="card"><h3>🎯 Tu meta</h3><p>Meta configurada en tu perfil</p><b class="v3metric">${p?.goal||700}</b></article><article class="card"><h3>📚 Material oficial</h3><p>Revisa pruebas y modelos DEMRE anteriores.</p><button class="secondary" onclick="v3DemreLibrary()">Abrir biblioteca</button></article><article class="card"><h3>📊 Diagnóstico adaptativo</h3><p>Analiza habilidades, ejes y dificultades de tus sesiones.</p><button class="secondary" onclick="diagnostic()">Ver diagnóstico</button></article></section>`;window.scrollTo(0,0);}
...
window.v3Home=v3Home;window.v3Training=v3Training;window.v3Exams=v3Exams;window.v3Progress=v3Progress;window.v3Profile=v3Profile;window.saveOfficialScore=saveOfficialScore;window.v3StartAdaptive=v3StartAdaptive;window.v3StartReview=v3StartReview;window.v3LearnTarget=v3LearnTarget;
window.home=v3Home;
